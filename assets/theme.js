...
(function () {
    ...
    // iOS fallback: show waifu over content for 'hold', then expand disk + switch theme
    function toggleIOS(next) {
      ...
      // Overlay container (no background – keeps page visible)
      ...
      // 1) Waifu on top of content (color depends on NEXT theme)
      var img = new Image();
      ...
      // Want: when switching to LIGHT, waifu should be white; to DARK: black.
      // If your GIF is black by default, invert for light.
      img.style.filter = (next === "light") ? "invert(1)" : "none";
      overlay.appendChild(img);
  
      // quick pop
      img.style.animation = `ios-waifu-pop ${popDur} ${ease} both`;
  
      // 2) After HOLD, expand a disk of the NEW theme's background and switch theme
      setTimeout(function(){
        ...
        // Switch theme exactly at the start of expand so var(--bg) becomes new color
        root.classList.add("theme-animating");
        root.dataset.theme = next;
        root.style.colorScheme = next;
        try { localStorage.setItem("theme", next); } catch (e) {}
